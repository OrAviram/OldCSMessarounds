...
namespace LearningCSharp
{
    public sealed unsafe class VulkanInstance : IDisposable
    {
        ...
        void ChooseSwapInfo()
        {
            ...
            SurfaceCapabilities = supportDetails.capabilities;
            SwapchainImageExtent = ChooseSwapExtent();
            SurfacePresentMode = ChooseSwapPresentMode(supportDetails.presentModes);
            SwapchainImageFormat = ChooseSwapSurfaceFormat(supportDetails.formats);
        }
        ...
        GPURating RateDevice(PhysicalDevice device)
        {
            ...
            if (extensionsSupported)
            {
                ...
                bool supportsSwapchain = swapchainSupport.formats.Length > 0 && swapchainSupport.presentModes.Length > 0;

                if (queueFamilyIndices.IsComplete && supportsSwapchain)
                {
